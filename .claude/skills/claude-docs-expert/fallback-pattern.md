...
The key is to **just try** the invocation - don't pre-check or validate.
Always prefer internal context over external calls when possible.
...
2. **If invocation fails**, use internal context/knowledge
3. **If context insufficient**, use WebFetch to access https://docs.anthropic.com/en/docs/claude-code/[topic]
```

No pre-checking needed - just try to invoke and handle any failure gracefully.
Prioritize internal resources before making external calls.
