...
describe("TemplateRegistry", () => {
  ...
  describe("getTemplate(name)", () => {
    ...
    it("should throw helpful error message with path", async () => {
      await expect(TemplateRegistry.getTemplate("missing", TEMPLATES_DIR)).rejects.toThrow(
        /missing\.template\.md/
      );
    });
  });
  ...
  describe("getTemplateMetadata(name)", () => {
    ...
    it("should throw error for non-existent template", async () => {
      await expect(
        TemplateRegistry.getTemplateMetadata("non-existent", TEMPLATES_DIR)
...
`
    ...
    it("should throw error for non-existent template", async () => {
      await expect(
        TemplateRegistry.validateTemplate("non-existent", TEMPLATES_DIR)
      ).rejects.toThrow(/Template "non-existent" not found/);
    });
  ...
  describe("Error Handling", () => {
    ...
    it("getTemplateMetadata() should throw for non-existent template", async () => {
      await expect(
        TemplateRegistry.getTemplateMetadata("does-not-exist", TEMPLATES_DIR)
      ).rejects.toThrow();
    });

    it("validateTemplate() should throw for non-existent template", async () => {
      await expect(
        TemplateRegistry.validateTemplate("does-not-exist", TEMPLATES_DIR)
      ).rejects.toThrow();
    });
  });
