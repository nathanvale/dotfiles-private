...
describe('Template System Integration Tests', () => {
  ...
  describe('External Plugin Discovery Pattern', () => {
    ...
    it('should allow plugin to pick a template and retrieve structure', async () => {
      ...
      const content = await TemplateRegistry.getTemplate(selectedTemplate.templateName, TEMPLATES_DIR)
...
`
  ...
  describe('Metadata Consistency', () => {
    it('metadata from listTemplates() should match getTemplateMetadata()', async () => {
      ...
      for (const listMetadata of templates) {
        const getMetadata = await TemplateRegistry.getTemplateMetadata(listMetadata.templateName, TEMPLATES_DIR)
  ...
  describe('Error Propagation', () => {
    it('error from missing template should flow through API consistently', async () => {
      ...
      await expect(TemplateRegistry.getTemplate(missingTemplateName, TEMPLATES_DIR)).rejects.toThrow(
        /Template "does-not-exist" not found/
      )

      await expect(TemplateRegistry.getTemplateMetadata(missingTemplateName, TEMPLATES_DIR)).rejects.toThrow(
        /Template "does-not-exist" not found/
      )

      await expect(TemplateRegistry.validateTemplate(missingTemplateName, TEMPLATES_DIR)).rejects.toThrow(
        /Template "does-not-exist" not found/
      )
    })
