...
describe("validate-templates CLI", () => {
  describe("with all valid templates", () => {
    ...
    it("should indicate all enrichments are present", async () => {
      ...
      expect(result.stdout).toMatch(
        /valid\.template\.md.*all.*enrichments|valid\.template\.md.*passed/i
      );
    });
  });
  ...
  describe("with invalid templates", () => {
    beforeEach(() => {
      mkdirSync(TEST_TEMPLATES_DIR, { recursive: true });
      writeFileSync(
        join(TEST_TEMPLATES_DIR, "invalid-location.template.md"),
        INVALID_TEMPLATE_MISSING_LOCATION
      );
    });
  ...
  describe("with missing Enrichment #5 dimension", () => {
    beforeEach(() => {
      mkdirSync(TEST_TEMPLATES_DIR, { recursive: true });
      writeFileSync(
        join(TEST_TEMPLATES_DIR, "invalid-blast-radius.template.md"),
        INVALID_TEMPLATE_MISSING_BLAST_RADIUS
      );
    });
  ...
  describe("with mixed valid and invalid templates", () => {
    beforeEach(() => {
      ...
      writeFileSync(join(TEST_TEMPLATES_DIR, "valid.template.md"), VALID_TEMPLATE);
      writeFileSync(
        join(TEST_TEMPLATES_DIR, "invalid.template.md"),
        INVALID_TEMPLATE_MISSING_LOCATION
      );
    });
  ...
  describe("output format", () => {
    beforeEach(() => {
      ...
      writeFileSync(
        join(TEST_TEMPLATES_DIR, "test2.template.md"),
        INVALID_TEMPLATE_MISSING_LOCATION
      );
    });
