#!/usr/bin/env tsx
/**
 * Validates that a generic has all 10 required enrichments
 *
 * Usage:
 *   tsx scripts/validate-generic.ts <generic-file.md>
 *   tsx scripts/validate-generic.ts --stdin < generic.md
 *
 * Exit codes:
...
class GenericValidator {
  ...
  validate(): GenericValidation {
    ...
    return {
      generic: this.generic.substring(0, 100) + '...',
      results,
      passed: errors === 0,
      errors,
      warnings,
  ...
  private validateComponent(): ValidationResult {
    const hasComponent = /\*\*Component:\*\*\s+C\d{2}:/i.test(this.generic)
    return {
      passed: hasComponent,
  ...
  private validateFileLocations(): ValidationResult {
    const hasLocation = /\*\*Location:\*\*\s+`[^`]+:\d+-\d+`/.test(this.generic)
    return {
      passed: hasLocation,
      message: hasLocation
  ...
  private validateEffortEstimate(): ValidationResult {
    const hasEffort = /\*\*Estimated Effort:\*\*\s+\d+(\.\d+)?h/.test(this.generic)
    return {
      passed: hasEffort,
      message: hasEffort
  ...
  private validateComplexity(): ValidationResult {
    const hasComplexity = /\*\*Complexity:\*\*\s+(CRITICAL|HIGH|MEDIUM|LOW)/.test(this.generic)
    return {
  ...
  private validateRegressionRisk(): ValidationResult {
    const hasRisk = /\*\*Regression Risk:\*\*\s+(HIGH|MEDIUM|LOW)/.test(this.generic)
    return {
      passed: hasRisk,
      message: hasRisk
  ...
  private validateRegressionRiskDetails(): ValidationResult {
    const hasImpact = /\*\*Impact:\*\*/.test(this.generic)
    const hasBlastRadius = /\*\*Blast Radius:\*\*/.test(this.generic)
    const hasDependencies = /\*\*Dependencies:\*\*/.test(this.generic)
    const hasTestingGaps = /\*\*Testing Gaps:\*\*/.test(this.generic)
    const hasRollbackRisk = /\*\*Rollback Risk:\*\*/.test(this.generic)
  ...
  private validateAcceptanceCriteria(): ValidationResult {
    const criteriaMatches = this.generic.match(/- \[ \]/g)
  ...
  private validateImplementationSteps(): ValidationResult {
    const stepsSection = this.generic.match(/\*\*Implementation Steps:\*\*([\s\S]*?)(?=\*\*|$)/i)
    if (!stepsSection) {
  ...
  private validateCodeExamples(): ValidationResult {
    const hasCurrent = /\*\*Current Code.*?\*\*:[\s\S]*?```/i.test(this.generic)
    const hasProposed = /\*\*Proposed.*?\*\*:[\s\S]*?```/i.test(this.generic)
  ...
  private validateFileChangeScope(): ValidationResult {
    const hasCreate = /\*\*Files to Create:\*\*/i.test(this.generic)
    const hasModify = /\*\*Files to Modify:\*\*/i.test(this.generic)
    const hasDelete = /\*\*Files to Delete:\*\*/i.test(this.generic)
  ...
  private validateTestingTable(): ValidationResult {
    ...
    const tableRows = this.generic.match(/\| (Unit|Integration|E2E)/gi)
  ...
  private validateDependencies(): ValidationResult {
    const hasBlocking = /\*\*Blocking Dependencies:\*\*/i.test(this.generic)
    const hasBlocks = /\*\*Blocks:\*\*/i.test(this.generic)
    const hasPrereqs = /\*\*Prerequisites:\*\*/i.test(this.generic)
...
Options:
  ...
  --stdin       Read generic from stdin instead of file
...
`)
  ...
  if (args.includes('--stdin')) {
    ...
    process.stdin.on('end', () => {
      generic = Buffer.concat(chunks).toString('utf-8')
      runValidation(generic)
    })
  } else {
    // Read from file
    ...
    try {
      generic = fs.readFileSync(filePath, 'utf-8')
      runValidation(generic)
    } catch (error) {
...
export { GenericValidator, ValidationResult, GenericValidation }
