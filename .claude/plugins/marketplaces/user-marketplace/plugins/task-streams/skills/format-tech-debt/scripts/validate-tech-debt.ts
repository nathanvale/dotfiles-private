#!/usr/bin/env tsx
/**
 * Validates that a techDebt has all 10 required enrichments
 *
 * Usage:
 *   tsx scripts/validate-techDebt.ts <techDebt-file.md>
 *   tsx scripts/validate-techDebt.ts --stdin < techDebt.md
 *
 * Exit codes:
...
class TechDebtValidator {
  private techDebt: string

  constructor(techDebt: string) {
    this.techDebt = techDebt
  }

  validate(): TechDebtValidation {
    ...
    return {
      techDebt: this.techDebt.substring(0, 100) + '...',
      results,
      passed: errors === 0,
      errors,
      warnings,
  ...
  private validateComponent(): ValidationResult {
    const hasComponent = /\*\*Component:\*\*\s+C\d{2}:/i.test(this.techDebt)
    return {
      passed: hasComponent,
  ...
  private validateFileLocations(): ValidationResult {
    const hasLocation = /\*\*Location:\*\*\s+`[^`]+:\d+-\d+`/.test(this.techDebt)
    return {
      passed: hasLocation,
      message: hasLocation
  ...
  private validateEffortEstimate(): ValidationResult {
    const hasEffort = /\*\*Estimated Effort:\*\*\s+\d+(\.\d+)?h/.test(this.techDebt)
    return {
      passed: hasEffort,
      message: hasEffort
  ...
  private validateComplexity(): ValidationResult {
    const hasComplexity = /\*\*Complexity:\*\*\s+(CRITICAL|HIGH|MEDIUM|LOW)/.test(this.techDebt)
    return {
  ...
  private validateRegressionRisk(): ValidationResult {
    const hasRisk = /\*\*Regression Risk:\*\*\s+(HIGH|MEDIUM|LOW)/.test(this.techDebt)
    return {
      passed: hasRisk,
      message: hasRisk
  ...
  private validateRegressionRiskDetails(): ValidationResult {
    const hasImpact = /\*\*Impact:\*\*/.test(this.techDebt)
    const hasBlastRadius = /\*\*Blast Radius:\*\*/.test(this.techDebt)
    const hasDependencies = /\*\*Dependencies:\*\*/.test(this.techDebt)
    const hasTestingGaps = /\*\*Testing Gaps:\*\*/.test(this.techDebt)
    const hasRollbackRisk = /\*\*Rollback Risk:\*\*/.test(this.techDebt)
  ...
  private validateAcceptanceCriteria(): ValidationResult {
    const criteriaMatches = this.techDebt.match(/- \[ \]/g)
  ...
  private validateImplementationSteps(): ValidationResult {
    const stepsSection = this.techDebt.match(/\*\*Implementation Steps:\*\*([\s\S]*?)(?=\*\*|$)/i)
    if (!stepsSection) {
      return { passed: false, message: '✗ Missing Implementation Steps section', severity: 'error' }
    }
  ...
  private validateCodeExamples(): ValidationResult {
    const hasCurrent = /\*\*Current Code.*?\*\*:[\s\S]*?```/i.test(this.techDebt)
    const hasProposed = /\*\*Proposed.*?\*\*:[\s\S]*?```/i.test(this.techDebt)
  ...
  private validateFileChangeScope(): ValidationResult {
    const hasCreate = /\*\*Files to Create:\*\*/i.test(this.techDebt)
    const hasModify = /\*\*Files to Modify:\*\*/i.test(this.techDebt)
    const hasDelete = /\*\*Files to Delete:\*\*/i.test(this.techDebt)
  ...
  private validateTestingTable(): ValidationResult {
    ...
    const tableRows = this.techDebt.match(/\| (Unit|Integration|E2E)/gi)
  ...
  private validateDependencies(): ValidationResult {
    const hasBlocking = /\*\*Blocking Dependencies:\*\*/i.test(this.techDebt)
    const hasBlocks = /\*\*Blocks:\*\*/i.test(this.techDebt)
    const hasPrereqs = /\*\*Prerequisites:\*\*/i.test(this.techDebt)
...
Options:
  ...
  --stdin       Read techDebt from stdin instead of file
...
`)
  ...
  if (args.includes('--stdin')) {
    ...
    process.stdin.on('end', () => {
      techDebt = Buffer.concat(chunks).toString('utf-8')
      runValidation(techDebt)
    })
  } else {
    // Read from file
    ...
    try {
      techDebt = fs.readFileSync(filePath, 'utf-8')
      runValidation(techDebt)
    } catch (error) {
...
function runValidation(techDebt: string) {
  const validator = new TechDebtValidator(techDebt)
  const result = validator.validate()

  console.log('\n=== TechDebt Validation Results ===\n')
  console.log(`TechDebt: ${result.techDebt}`)
  console.log()
...
export { TechDebtValidator, ValidationResult, TechDebtValidation }
