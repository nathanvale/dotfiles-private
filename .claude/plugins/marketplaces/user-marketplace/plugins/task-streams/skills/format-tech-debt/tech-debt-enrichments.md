...
These 5 enrichments are unique to tech debt tasks and complement the 10 universal enrichments from SHARED_ENRICHMENTS.md.
...
**Cyclomatic Complexity**: 42 (target: <10)
**Test Coverage**: 34% (target: 80%)
**Maintainability Index**: 42/100 (LOW)
**Technical Debt Ratio**: 8.2% (SonarQube)
**Duplicated Code**: 456 lines (8.3%)
```
...
**Current Cost**: $197k/quarter (bug fixes, incidents, velocity loss)
**Investment**: $65k (one-time)
**Savings**: $163k/quarter
...
**SOLID Violations Fixed**: Single Responsibility, Dependency Injection
**Test Coverage**: 34% → 95%
**Complexity**: 42 → 8 (cyclomatic)
```
...
**Parallel Run**: Compare old vs new (shadow mode)
**Rollback Plan**: Feature flag flip (<5 min)
**Rollout**: Canary 5% → 25% → 50% → 100%
**Auto-rollback If**: Error rate >0.5%, latency >500ms
```
