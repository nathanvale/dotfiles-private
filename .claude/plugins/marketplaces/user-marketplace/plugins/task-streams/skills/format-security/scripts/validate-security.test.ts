/**
 * Tests for security security validation
 */

import { describe, it, expect } from 'vitest'

import { SecurityValidator } from './validate-security'

describe('SecurityValidator', () => {
  describe('Complete valid security', () => {
    it('should pass all validations for properly enriched security', () => {
      const validSecurity = `
...
`
      const validator = new SecurityValidator(validSecurity)
...
**Estimated Effort:**
    ...
    it('should pass with various effort formats', () => {
      ...
      for (const effort of testCases) {
        const security = `**Estimated Effort:** ${effort}`
        const validator = new SecurityValidator(security)
        const result = validator.validate()
  ...
  describe('Complexity validation', () => {
    it('should pass with all valid complexity levels', () => {
      ...
      for (const level of levels) {
        const security = `**Complexity:** ${level}`
        const validator = new SecurityValidator(security)
        const result = validator.validate()
    ...
    it('should fail with invalid complexity level', () => {
      const security = `**Complexity:** SUPER_HIGH`
      const validator = new SecurityValidator(security)
      const result = validator.validate()
