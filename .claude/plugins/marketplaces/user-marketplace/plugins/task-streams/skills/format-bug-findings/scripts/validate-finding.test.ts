/**
 * Tests for bug finding validation
 */

import { describe, it, expect } from "vitest";

import { FindingValidator } from "./validate-finding";

describe("FindingValidator", () => {
  describe("Complete valid finding", () => {
    it("should pass all validations for properly enriched finding", () => {
      const validFinding = `
...
`;
      const validator = new FindingValidator(validFinding);
...
**Estimated Effort:**
    ...
    it("should pass with various effort formats", () => {
      ...
      for (const effort of testCases) {
        const finding = `**Estimated Effort:** ${effort}`;
        const validator = new FindingValidator(finding);
        const result = validator.validate();
  ...
  describe("Complexity validation", () => {
    it("should pass with all valid complexity levels", () => {
      ...
      for (const level of levels) {
        const finding = `**Complexity:** ${level}`;
        const validator = new FindingValidator(finding);
        const result = validator.validate();
    ...
    it("should fail with invalid complexity level", () => {
      const finding = `**Complexity:** SUPER_HIGH`;
      const validator = new FindingValidator(finding);
      const result = validator.validate();
