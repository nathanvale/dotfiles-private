#!/usr/bin/env tsx
/**
 * Validates that a spec has all 10 required enrichments
 *
 * Usage:
 *   tsx scripts/validate-spec.ts <spec-file.md>
 *   tsx scripts/validate-spec.ts --stdin < spec.md
 *
 * Exit codes:
...
class SpecValidator {
  ...
  validate(): SpecValidation {
    ...
    return {
      spec: this.spec.substring(0, 100) + '...',
      results,
      passed: errors === 0,
      errors,
      warnings,
  ...
  private validateComponent(): ValidationResult {
    const hasComponent = /\*\*Component:\*\*\s+C\d{2}:/i.test(this.spec)
    return {
      passed: hasComponent,
  ...
  private validateFileLocations(): ValidationResult {
    const hasLocation = /\*\*Location:\*\*\s+`[^`]+:\d+-\d+`/.test(this.spec)
    return {
      passed: hasLocation,
      message: hasLocation
  ...
  private validateEffortEstimate(): ValidationResult {
    const hasEffort = /\*\*Estimated Effort:\*\*\s+\d+(\.\d+)?h/.test(this.spec)
    return {
      passed: hasEffort,
      message: hasEffort
  ...
  private validateComplexity(): ValidationResult {
    const hasComplexity = /\*\*Complexity:\*\*\s+(CRITICAL|HIGH|MEDIUM|LOW)/.test(this.spec)
    return {
  ...
  private validateRegressionRisk(): ValidationResult {
    const hasRisk = /\*\*Regression Risk:\*\*\s+(HIGH|MEDIUM|LOW)/.test(this.spec)
    return {
      passed: hasRisk,
      message: hasRisk
  ...
  private validateRegressionRiskDetails(): ValidationResult {
    const hasImpact = /\*\*Impact:\*\*/.test(this.spec)
    const hasBlastRadius = /\*\*Blast Radius:\*\*/.test(this.spec)
    const hasDependencies = /\*\*Dependencies:\*\*/.test(this.spec)
    const hasTestingGaps = /\*\*Testing Gaps:\*\*/.test(this.spec)
    const hasRollbackRisk = /\*\*Rollback Risk:\*\*/.test(this.spec)
  ...
  private validateAcceptanceCriteria(): ValidationResult {
    const criteriaMatches = this.spec.match(/- \[ \]/g)
  ...
  private validateImplementationSteps(): ValidationResult {
    const stepsSection = this.spec.match(/\*\*Implementation Steps:\*\*([\s\S]*?)(?=\*\*|$)/i)
    if (!stepsSection) {
  ...
  private validateCodeExamples(): ValidationResult {
    const hasCurrent = /\*\*Current Code.*?\*\*:[\s\S]*?```/i.test(this.spec)
    const hasProposed = /\*\*Proposed.*?\*\*:[\s\S]*?```/i.test(this.spec)
  ...
  private validateFileChangeScope(): ValidationResult {
    const hasCreate = /\*\*Files to Create:\*\*/i.test(this.spec)
    const hasModify = /\*\*Files to Modify:\*\*/i.test(this.spec)
    const hasDelete = /\*\*Files to Delete:\*\*/i.test(this.spec)
  ...
  private validateTestingTable(): ValidationResult {
    ...
    const tableRows = this.spec.match(/\| (Unit|Integration|E2E)/gi)
  ...
  private validateDependencies(): ValidationResult {
    const hasBlocking = /\*\*Blocking Dependencies:\*\*/i.test(this.spec)
    const hasBlocks = /\*\*Blocks:\*\*/i.test(this.spec)
    const hasPrereqs = /\*\*Prerequisites:\*\*/i.test(this.spec)
...
Options:
  ...
  --stdin       Read spec from stdin instead of file
...
function runValidation(spec: string) {
...
export { SpecValidator, ValidationResult, SpecValidation }
