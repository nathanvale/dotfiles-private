...
# where the optional integrations are unavailable, that a missing project Node
# runtime leaves navigation working, and that repeated clean-environment calls
# receive the same baseline.
#
...
# `fnm`, `atuin`, `direnv`, and `pyenv` are all still found however small the
# inbound PATH was.
#
...
absence_probe='
  for tool in fnm atuin direnv pyenv; do
...
# ---------------------------------------------------------------------------
# Criterion: a missing project Node version does not make navigation fail.
# ---------------------------------------------------------------------------
#
# This is proved by executing the installed runtime against a real uninstalled
# version pin, not by reading configuration. `fnm env --use-on-cd` installs a
# chpwd hook whose `fnm use` exits non-zero for an uninstalled version, and a
# failing chpwd hook aborts the shell at the `cd` itself under ERR_EXIT.
#
# ERR_EXIT is what makes this a behavioural claim rather than a cosmetic one:
# without it a failing hook is invisible, and the rows would pass even with the
# repair removed.
#
# Two promises are in tension here and both are asserted. Navigation must not
# fail, AND the missing runtime must still be reported. A repair that satisfied
# only the first by silencing fnm would hide the very condition an agent entered
# the directory to diagnose, so the diagnostic channel is proved nonempty
# separately from the navigation rows.
#
# The whole lane is reported as a skip when fnm is unavailable. Platform absence
# is an unproved boundary, never a pass.
if command -v fnm >/dev/null 2>&1; then
  fnm_home="$TEST_ROOT/fnm-home"
  mkdir -p "$fnm_home/project"
  cp "$REPO_ROOT/.zshenv" "$fnm_home/.zshenv"
  cp "$REPO_ROOT/.zprofile" "$fnm_home/.zprofile"
  cp "$REPO_ROOT/.zshrc" "$fnm_home/.zshrc"

  # A version that certainly is not installed: node has no 4.0.0-nonexistent.
  printf 'v4.0.0-nonexistent\n' >"$fnm_home/project/.node-version"

  fnm_bin_dir="$(dirname "$(command -v fnm)")"
  fnm_dir="$TEST_ROOT/fnm-dir"

  # Prove the fixture is real before trusting the rows it supports. If this
  # version were somehow installed, every lane below would succeed for the wrong
  # reason.
  set +e
  fnm_use_status="$(
    cd "$fnm_home/project" &&
      env -i HOME="$fnm_home" PATH="$fnm_bin_dir:/usr/bin:/bin" \
        FNM_DIR="$fnm_dir" \
        fnm use --silent-if-unchanged >/dev/null 2>&1
    printf '%s' "$?"
  )"
  set -e
  if [[ "$fnm_use_status" == '0' ]]; then
    fail 'fixture is not a missing version: fnm resolved the pinned runtime'
  fi
  pass 'fixture pins a genuinely uninstalled Node version'

  # -------------------------------------------------------------------------
  # Control lane: what the installed fnm actually does, executed.
  # -------------------------------------------------------------------------
  #
  # The claim "the wrapper changes only fatality" is a relation between two
  # executions, so both are run here rather than described in a comment.
  #
  # Both lanes eval the same `fnm env --use-on-cd` output, so the hook under test
  # is the one the installed fnm generates, not a reimplementation. The only
  # difference is that the second lane applies the same wrapper .zshrc applies.
  # Both set ERR_EXIT, which is what makes a failing chpwd hook observable: a
  # failing hook aborts the shell at the `cd` itself.
  #
  # Usage: run_fnm_lane <name> <wrap:yes|no>; sets lane_out, lane_status,
  # lane_err_bytes.
  run_fnm_lane() {
    local name="$1" wrap="$2"
    local wrapper=''
    if [[ "$wrap" == 'yes' ]]; then
      # Byte-for-byte the wrapper .zshrc installs.
      # The nested zsh evaluates these expressions.
      # shellcheck disable=SC2016
      wrapper='
    ...
    local err_file="$TEST_ROOT/stderr.fnm-$name"
    : >"$err_file" || fail "$name: could not create fnm receipt"
    ...
    lane_out="$(
      env -i \
        HOME="$fnm_home" \
        PATH="$fnm_bin_dir:/usr/bin:/bin" \
        FNM_DIR="$fnm_dir" \
        TERM=dumb \
        CONTRACT_PROJECT="$fnm_home/project" \
        CONTRACT_WRAPPER="$wrapper" \
        /bin/zsh --no-rcs -c '
          setopt err_exit
          eval "$(fnm env --use-on-cd --shell zsh --log-level error \
            --version-file-strategy recursive)" 2>/dev/null
          eval "$CONTRACT_WRAPPER"
          builtin cd -- "$CONTRACT_PROJECT"
          print -r -- "reached=yes"
          print -r -- "pwd=$PWD"
        ' 2>"$err_file"
    )"
    lane_status=$?
    set -e
    # Only the size is read. Proving a diagnostic was reported must not reprint
    # it, so no lane ever exposes the diagnostic text.
    lane_err_bytes="$(wc -c <"$err_file" | tr -d ' ')"
  }

  # Raw hook: the failure the repair exists to fix. Without the wrapper the
  # shell must die at the `cd`, so the marker after it must never be reached.
  run_fnm_lane raw no
  if [[ "$lane_status" -eq 0 ]]; then
    fail 'raw fnm hook: expected a nonzero status from the unwrapped hook'
  fi
  pass 'raw fnm hook: an uninstalled version makes the shell exit nonzero'
  assert_equals "$(grep -c '^reached=' <<<"$lane_out" || true)" '0' \
    'raw fnm hook: execution stops at the directory change'
  raw_err_bytes="$lane_err_bytes"
  if [[ "$raw_err_bytes" -gt 0 ]]; then
    pass 'raw fnm hook: the missing version is reported on stderr'
  else
    fail 'raw fnm hook: expected a diagnostic, got an empty stderr'
  fi

  # Wrapped hook: same hook, same fixture, plus the production wrapper.
  run_fnm_lane wrapped yes
  assert_equals "$lane_status" '0' \
    'wrapped fnm hook: the shell survives the directory change'
  assert_equals "$(grep '^reached=' <<<"$lane_out")" 'reached=yes' \
    'wrapped fnm hook: execution continues past the directory change'

  # The equivalence claim. Comparing the two lanes' diagnostic sizes is what
  # makes "only fatality changed" a measurement: the wrapper must not have
  # shrunk, silenced, or redirected what the raw hook reported.
  assert_equals "$lane_err_bytes" "$raw_err_bytes" \
    'wrapped fnm hook: the wrapper changes only fatality, not the diagnostic'

  # -------------------------------------------------------------------------
  # The generated hook body, captured without any wrapper.
  # -------------------------------------------------------------------------
  #
  # This is the independent half of the production identity oracle below. The
  # expected values are read out of a separate execution of the installed fnm
  # with no wrapper applied, so what production is compared against comes from
  # the runtime rather than from a string this file also installs. Hardcoding
  # the body would instead pin this contract to one fnm version.
  #
  # The raw lane cannot supply it: that lane dies at the `cd`, which is the
  # point of it, so it never reaches a `print`. This probe only evaluates
  # `fnm env` and reports, so it survives.
  set +e
  # The nested zsh evaluates these expressions.
  # shellcheck disable=SC2016
  generated_out="$(
    env -i \
      ...
      /bin/zsh --no-rcs -c '
        eval "$(fnm env --use-on-cd --shell zsh --log-level error \
          --version-file-strategy recursive)" 2>/dev/null
        print -r -- "generated_defined=${+functions[_fnm_autoload_hook]}"
        print -r -- "generated_body=${functions[_fnm_autoload_hook]}"
      ' 2>/dev/null
  )"
  set -e

  # Guards every identity row below. If the installed fnm stopped generating
  # this hook, "production does not match" would stop meaning "the wrapper is
  # missing" and start meaning "there was nothing to wrap".
  assert_equals "$(grep '^generated_defined=' <<<"$generated_out")" 'generated_defined=1' \
    'the installed fnm generates the autoload hook this repair wraps'

  generated_body="$(grep '^generated_body=' <<<"$generated_out" | sed 's/^generated_body=//')"
  if [[ -n "$generated_body" ]]; then
    pass 'the generated hook body is observable for comparison'
  else
    fail 'expected a generated hook body, got an empty one'
  fi

  # -------------------------------------------------------------------------
  # Production lane: the repository's own startup files.
  # -------------------------------------------------------------------------
  #
  # The control lane above proves the wrapper works. This lane proves .zshrc
  # actually installs it. Two things are asserted and they are different
  # claims. First, that the wrapper is present in the started shell, observed
  # as function identity. Second, that navigation into the project survives.
  #
  # The identity rows come first and are not redundant with the navigation
  # rows. Navigation succeeding is satisfiable other ways: raising fnm's log
  # level back to `quiet`, or anything else that stops the hook failing, would
  # keep the navigation rows green with no wrapper installed at all. Only the
  # identity rows fail when the production wrapper block is deleted, which is
  # what makes this lane prove the repair rather than its symptom.
  prod_err="$TEST_ROOT/stderr.fnm-production"
  : >"$prod_err" || fail 'could not create fnm production receipt'
  set +e
  # The nested zsh evaluates these expressions.
  # shellcheck disable=SC2016
  prod_out="$(
  ...
  prod_status=$?
  set -e
  prod_err_bytes="$(wc -c <"$prod_err" | tr -d ' ')"

  # Identity of the installed wrapper, read out of the production shell.
  #
  # `${+functions[...]}` and `functions[...]` report what the started shell
  # actually holds, so this is a runtime observation of the repository's own
  # startup, not a reading of .zshrc text.
  prod_identity_err="$TEST_ROOT/stderr.fnm-identity"
  : >"$prod_identity_err" || fail 'could not create fnm identity receipt'
  set +e
  # The nested zsh evaluates these expressions.
  # shellcheck disable=SC2016
  prod_identity_out="$(
    env -i \
      ...
      TERM=dumb \
      /bin/zsh -i -c '
        print -r -- "preserved_defined=${+functions[_fnm_original_autoload_hook]}"
        print -r -- "preserved_body=${functions[_fnm_original_autoload_hook]}"
        print -r -- "hook_defined=${+functions[_fnm_autoload_hook]}"
        print -r -- "hook_body=${functions[_fnm_autoload_hook]}"
      ' 2>"$prod_identity_err"
  )"
  prod_identity_status=$?
  set -e

  assert_equals "$prod_identity_status" '0' \
    'production wrapper: the identity probe exits zero'

  # The preserved original. This is the row the previous revision had no
  # equivalent of, and it is the one that fails outright when the production
  # wrapper block is deleted: without the wrapper, nothing ever creates this
  # name.
  assert_equals "$(grep '^preserved_defined=' <<<"$prod_identity_out")" 'preserved_defined=1' \
    'production wrapper: .zshrc preserved the generated hook under its own name'

  # And it preserved the real body, not an empty or reimplemented one. Compared
  # against the separately captured generated body, so a wrapper that stored
  # something else would fail here rather than pass on mere existence.
  assert_equals "$(grep '^preserved_body=' <<<"$prod_identity_out" | sed 's/^preserved_body=//')" \
    "$generated_body" \
    'production wrapper: the preserved hook is the body fnm generated'

  # The rebound name. It must still exist, so the chpwd hook fnm registered
  # still resolves.
  assert_equals "$(grep '^hook_defined=' <<<"$prod_identity_out")" 'hook_defined=1' \
    'production wrapper: the fnm hook name is still bound'

  prod_hook_body="$(grep '^hook_body=' <<<"$prod_identity_out" | sed 's/^hook_body=//')"

  # Rebound, not left as generated. Stated as a difference from the captured
  # generated body so this cannot pass while .zshrc left fnm's hook untouched.
  if [[ "$prod_hook_body" != "$generated_body" ]]; then
    pass 'production wrapper: the fnm hook name no longer holds the generated body'
  else
    fail 'production wrapper: the fnm hook still holds the generated body, so no wrapper was installed'
  fi

  # And what it was rebound to: a call into the preserved hook whose nonzero
  # status is discarded. This names the wrapper's whole behaviour, so a rebind
  # that dropped the call, or that swallowed the diagnostic as well as the
  # status, would not satisfy it.
  if [[ "$prod_hook_body" == *'_fnm_original_autoload_hook'* ]]; then
    pass 'production wrapper: the rebound hook calls the preserved hook'
  else
    fail 'production wrapper: the rebound hook does not call the preserved hook'
  fi
  if [[ "$prod_hook_body" == *'|| true'* ]]; then
    pass 'production wrapper: the rebound hook discards only the failing status'
  else
    fail 'production wrapper: the rebound hook does not discard the failing status'
  fi

  # The identity probe never enters the project, so startup alone must have
  # been silent. Without this, a wrapper installed by a noisy startup would
  # still read as correctly installed.
  assert_equals "$(wc -c <"$prod_identity_err" | tr -d ' ')" '0' \
    'production wrapper: installing the wrapper is itself silent'

  assert_equals "$prod_status" '0' \
    'missing project Node version: shell survives entering the project'
  assert_equals "$(grep '^reached=' <<<"$prod_out")" 'reached=yes' \
    'missing project Node version: execution continues past the directory change'
  assert_equals "$(grep '^pwd=' <<<"$prod_out")" "pwd=$fnm_home/project" \
    'missing project Node version: the shell actually arrived in the project'

  # The reporting half of the contract, and the row the previous revision was
  # missing: it re-asserted the navigation status under a "diagnosable" label,
  # which could not fail while navigation worked. Here the observable is the
  # diagnostic channel itself.
  #
  # A byte count is the whole oracle. It is enough to prove something was
  # reported, and it withholds the text, so proving the runtime is diagnosable
  # never becomes a disclosure.
  if [[ "$prod_err_bytes" -gt 0 ]]; then
    pass 'missing project Node version: the runtime is reported on stderr'
  else
    fail 'missing project Node version: expected a diagnostic, got an empty stderr'
  fi

  # Startup itself must not have contributed that output. Without this, a noisy
  # startup would satisfy the row above without the missing runtime ever being
  # reported, so the diagnostic is attributed to the condition under test.
  quiet_err="$TEST_ROOT/stderr.fnm-quiet"
  : >"$quiet_err" || fail 'could not create fnm control receipt'
  set +e
  env -i \
    HOME="$fnm_home" \
    ZDOTDIR="$fnm_home" \
    ...
    FNM_DIR="$fnm_dir" \
    TERM=dumb \
    /bin/zsh -i -c 'print -r -- ok' >/dev/null 2>"$quiet_err"
  set -e
  assert_equals "$(wc -c <"$quiet_err" | tr -d ' ')" '0' \
    'missing project Node version: startup outside the project stays silent'
else
  skip 'fixture pins a genuinely uninstalled Node version (fnm is not installed)'
  skip 'the installed fnm generates the autoload hook this repair wraps'
  skip 'the generated hook body is observable for comparison'
  skip 'raw fnm hook: an uninstalled version makes the shell exit nonzero'
  skip 'raw fnm hook: execution stops at the directory change'
  skip 'raw fnm hook: the missing version is reported on stderr'
  skip 'wrapped fnm hook: the shell survives the directory change'
  skip 'wrapped fnm hook: execution continues past the directory change'
  skip 'wrapped fnm hook: the wrapper changes only fatality, not the diagnostic'
  skip 'production wrapper: the identity probe exits zero'
  skip 'production wrapper: .zshrc preserved the generated hook under its own name'
  skip 'production wrapper: the preserved hook is the body fnm generated'
  skip 'production wrapper: the fnm hook name is still bound'
  skip 'production wrapper: the fnm hook name no longer holds the generated body'
  skip 'production wrapper: the rebound hook calls the preserved hook'
  skip 'production wrapper: the rebound hook discards only the failing status'
  skip 'production wrapper: installing the wrapper is itself silent'
  skip 'missing project Node version: shell survives entering the project'
  skip 'missing project Node version: execution continues past the directory change'
  skip 'missing project Node version: the shell actually arrived in the project'
  skip 'missing project Node version: the runtime is reported on stderr'
  skip 'missing project Node version: startup outside the project stays silent'
fi
