...
- `.fallowrc.json` `ignoreDependencies` holds `vscode` because the VS Code extension host supplies it and unlisted-dependency findings have no inline suppression path. Keep `@logtape/logtape`, `@logtape/redaction`, `@logtape/file`, and `zod` because the playground's nested, independently locked packages (`recovery-observability`, `cli-design-exemplar`) declare them and root workspace discovery never registers those packages. Each name is an exact package name that is declared in the importing package's `package.json`, resolved in the playground `bun.lock`, and materialized under that package's `node_modules` (`@logtape/*@2.3.1`, `zod@4.4.3` for `cli-design-exemplar` on 2026-09-14); add a name only with that three-part evidence, never for an import the package does not declare. Fallow `workspaces.patterns` was tested with the playground root and with `packages/*` on 2026-09-12 and again on 2026-09-14 with the exact `packages/cli-design-exemplar` path; `fallow list --workspaces` reported the same 13 workspaces with no diagnostic each time, and the unlisted-dependency findings persisted, so the allowance is the only configuration path short of registering the playground under root `workspaces`. Known cost: `ignoreDependencies` also excludes those names from unused-dependency detection in registered workspaces; on 2026-09-14 only `.agents/runtime/cli-command-facade` declares one of them (`@logtape/logtape`) and its rooted run proves it used. Keep the playground `entry` glob because Fallow does not read `bun build <path>` script arguments as entries. Registering the playground under root `workspaces` remains untested and would change its independent lock ownership.
