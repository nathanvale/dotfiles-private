...
const chordActions = [
  ...
  { key: 'D', label: 'Diff/source toggle', command: 'nathan.toggleGitDiff', receipt: false },
  { key: ',', label: 'Changed files', command: 'nathan.showChangedFiles', receipt: false },
  ...
  { key: 'T', label: 'Terminal', command: 'workbench.action.terminal.toggleTerminal' },
  { key: 'C', label: 'Chat', command: 'workbench.action.openChat' },
  { key: 'W', label: 'Close all editors', command: 'workbench.action.closeAllEditors' },
  { key: 'Z', label: 'Zen mode', command: 'workbench.action.toggleZenMode' },
  { key: 'H', label: 'Focus left group', command: 'workbench.action.focusLeftGroup' },
  { key: 'J', label: 'Focus below group', command: 'workbench.action.focusBelowGroup' },
  { key: 'K', label: 'Focus above group', command: 'workbench.action.focusAboveGroup' },
  { key: 'L', label: 'Focus right group', command: 'workbench.action.focusRightGroup' },
  ...
  { key: ';', label: 'Keyboard shortcuts', command: 'workbench.action.openGlobalKeybindings' },
  { key: 'M', label: 'Markdown source', command: 'markdown.showSource' },
  { key: 'R', label: 'Run task', command: 'workbench.action.tasks.runTask' },
  { key: 'Shift+R', label: 'Rerun task', command: 'workbench.action.tasks.reRunTask' },
  { key: 'Shift+T', label: 'Test task', command: 'workbench.action.tasks.test' },
...
function hasFileChange(repo, fileUri) {
  return getRepoChanges(repo).some((change) => sameUri(getChangeUri(change), fileUri))
}
...
function getFileUriFromTab(tab) {
  ...
  return getFileUri(input?.uri) ?? getFileUri(input?.modified) ?? getFileUri(input?.original)
}
...
function recordReceipt(message) {
  receiptHistory = [message, ...receiptHistory.filter((item) => item !== message)].slice(
    0,
    RECEIPT_LIMIT,
  )

  if (!receiptItem) {
    receiptItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 9998)
...
function flash(message, editor, tone = 'warning', suggestion) {
  ...
  flashItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 10000)
  flashItem.text =
    tone === 'success' ? `$(check) ${visibleMessage}` : `$(warning) ${visibleMessage}`
...
function ensureCheatSheetItem() {
  ...
  cheatSheetItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 10002)
  cheatSheetItem.text = '$(keyboard) Ctrl+G ?'
  cheatSheetItem.tooltip = 'Nathan ADHD Helper chord map. Click for searchable actions.'
...
function updateModeFrame(modeOverride) {
  ...
  if (!modeItem) {
    modeItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 9997)
...
async function refreshDirtyRadar() {
  ...
  const count = api
    ? api.repositories.reduce((total, repo) => total + getRepoChanges(repo).length, 0)
    : 0

  if (!dirtyItem) {
    dirtyItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 9996)
...
async function exitChordMode() {
  ...
  await vscode.commands.executeCommand('setContext', 'nathan.adhdChordMode', false)
}
...
function scheduleChordTimeout() {
  ...
  const timeout =
    chordMode === chordHudMode.EXPANDED ? CHORD_INSPECT_TIMEOUT_MS : CHORD_TIMEOUT_MS
...
function renderChordHud() {
  if (!chordItem) {
    chordItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 10001)
    chordItem.tooltip = formatChordTooltip()
    chordItem.backgroundColor = new vscode.ThemeColor('statusBarItem.prominentBackground')
    chordItem.show()
  }
...
async function enterChordMode(mode = chordHudMode.COMPACT) {
  await exitChordMode()
  await vscode.commands.executeCommand('setContext', 'nathan.adhdChordMode', true)
...
async function runChordAction(action) {
  ...
  if (!action || !action.command) {
    flash('Unknown chord action', undefined, 'warning', 'Press Ctrl+G to reopen HUD')
    return
  }
...
async function showChangedFiles() {
  ...
  if (!api) {
    flash('Git extension unavailable', undefined, 'warning', 'Open Source Control')
    return
  }
  ...
  if (items.length === 0) {
    flash('No changed files', undefined, 'warning', 'Ctrl+G G opens Source Control')
    await refreshDirtyRadar()
    return
  }
...
async function toggleGitDiff() {
  ...
  if (!fileUri) {
    flash('No source file to diff', undefined, 'warning', 'Ctrl+G , shows changed files')
    return
  }
  ...
  if (!repo) {
    flash('No Git repository for this file', editor, 'warning', 'Ctrl+G E opens Explorer')
    await refreshDirtyRadar()
    return
  }

  if (!hasFileChange(repo, fileUri)) {
    flash('No Git diff for this file', editor, 'warning', 'Ctrl+G , shows changed files')
    await refreshDirtyRadar()
    return
  }
...
async function openSourceFromDiff() {
  ...
  if (!sourceUri) {
    flash('No source file for this diff', undefined, 'warning', 'Ctrl+G , shows changed files')
    return
  }
  ...
  try {
    ...
    flash('Source file unavailable', undefined, 'warning', 'Ctrl+G , shows changed files')
    await refreshDirtyRadar()
    updateModeFrame()
    return
  }
...
function activate(context) {
  const subscriptions = [
    ...
    vscode.commands.registerCommand('nathan.expandChordHudForInspection', expandChordHudForInspection),
    vscode.commands.registerCommand('nathan.showChordPalette', showChordPalette),
    ...
    vscode.commands.registerCommand('nathan.openSourceFromDiff', openSourceFromDiff),
    vscode.commands.registerCommand('nathan.showChangedFiles', showChangedFiles),
