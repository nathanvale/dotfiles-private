...
show_active_locks() {
    printf "${BOLD}${BLUE}📋 Active Task Locks:${RESET}\n"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    if ! command -v "$taskdock_cmd" &> /dev/null && [ ! -x "$taskdock_cmd" ]; then
        printf "${YELLOW}  taskdock CLI not found${RESET}\n"
        echo
        return
    fi

    local locks_json
    locks_json=$("$taskdock_cmd" --json locks list 2>/dev/null || echo "[]")

    # Validate JSON
    if ! echo "$locks_json" | jq empty 2>/dev/null; then
        # If invalid JSON, try to extract JSON array from it (in case of mixed output)
        locks_json=$(echo "$locks_json" | sed -n '/^\[/,/^\]/p')
        if ! echo "$locks_json" | jq empty 2>/dev/null; then
             printf "${RED}Error: Invalid JSON output from taskdock${RESET}\n"
             return
        fi
    fi

    if [ "$locks_json" == "[]" ] || [ -z "$locks_json" ]; then
        printf "${YELLOW}  No active locks${RESET}\n"
        echo
        return
    fi
