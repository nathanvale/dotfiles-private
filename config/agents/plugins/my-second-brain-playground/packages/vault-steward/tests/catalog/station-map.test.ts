...
const scenarios: Scenario[] = [
	...
	{ name: "begin path through a symbolic link", run: () => { const f = fixture(); const outside = join(f.root, "outside"); mkdirSync(outside); symlinkSync(outside, join(f.vault, "linked")); const result = run(f, ["begin", "--vault", f.vault, "--path", "linked/note.md"]); expect(existsSync(join(outside, "note.md"))).toBe(false); return result }, cause: "DOMAIN_PATH_REFUSED", exit: 3, nextAction: BEGIN },
	{ name: "begin completed from the configured vault", run: () => { const f = fixture(); return run(f, ["begin", "--path", "projects/demo/GOAL.md"], withConfig(f, JSON.stringify({ schemaVersion: 1, vault: f.vault }))) }, cause: "SUCCESS_COMPLETED", exit: 0, nextAction: PREVIEW, transaction: "completed" },
	{ name: "begin relative state home", run: () => { const f = fixture(); const result = run(f, ["begin", "--vault", f.vault, "--path", "a.md"], { XDG_STATE_HOME: "relative-state" }); expect(existsSync(join(f.vault, "relative-state"))).toBe(false); return result }, cause: "SCHEMA_INVALID_INPUT", exit: 4, nextAction: HELP },
	...
	{ name: "preview forged receipt commit", run: () => { const f = fixture(); const { worktree } = applied(f); const receipt = receiptPathOf(f, worktree); writeFileSync(receipt, JSON.stringify({ ...JSON.parse(readFileSync(receipt, "utf8")), commit: git(f.vault, "rev-parse", "HEAD^") })); return run(f, ["finish", "--preview", "--worktree", worktree, "--message", "x"]) }, cause: "SCHEMA_RECEIPT_INVALID", exit: 4, handoffOwner: "operator" },
	{ name: "preview receipt identity mismatch", run: () => { const f = fixture(); const { worktree } = applied(f); const receipt = receiptPathOf(f, worktree); writeFileSync(receipt, JSON.stringify({ ...JSON.parse(readFileSync(receipt, "utf8")), worktree: `${worktree}/../other` })); return run(f, ["finish", "--preview", "--worktree", worktree, "--message", "x"]) }, cause: "SCHEMA_RECEIPT_INVALID", exit: 4, handoffOwner: "operator" },
	{ name: "preview receipt through a symbolic link", run: () => { const f = fixture(); const { worktree } = applied(f); const receipt = receiptPathOf(f, worktree); const original = readFileSync(receipt, "utf8"); const elsewhere = join(f.root, "elsewhere.json"); writeFileSync(elsewhere, original, { mode: 0o600 }); rmSync(receipt); symlinkSync(elsewhere, receipt); const result = run(f, ["finish", "--preview", "--worktree", worktree, "--message", "x"]); expect(readFileSync(elsewhere, "utf8")).toBe(original); return result }, cause: "SCHEMA_RECEIPT_INVALID", exit: 4, handoffOwner: "operator" },
	...
	{ name: "preview check failed on a pre-committed candidate", run: () => { const f = fixture(); const w = beginCandidate(f, "BROKEN", "fail\n"); git(w, "add", "--", "BROKEN"); git(w, "commit", "-m", "docs: candidate created outside the CLI"); const result = run(f, ["finish", "--preview", "--worktree", w, "--message", "x"]); expect(git(f.vault, "rev-parse", "HEAD")).toBe(f.initialHead); return result }, cause: "DOMAIN_CHECK_FAILED", exit: 3, nextAction: PREVIEW },
	{ name: "preview partial path set", run: () => { const f = fixture(); const w = (data(run(f, ["begin", "--vault", f.vault, "--path", "projects/demo/GOAL.md", "--path", "products/lamp.md"])).candidate as { worktree: string }).worktree; write(w, "products/lamp.md", "# Lamp\n"); return run(f, ["finish", "--preview", "--worktree", w, "--message", "x"]) }, cause: "DOMAIN_PATH_SET_MISMATCH", exit: 3, nextAction: PREVIEW },
	{ name: "preview checker mutated an unadmitted file", run: () => { const f = fixture(); write(f.vault, "check.ts", 'import { writeFileSync } from "node:fs"\nwriteFileSync("README.md", "checker mutation\\n")\n'); git(f.vault, "add", "--", "check.ts"); git(f.vault, "commit", "-m", "test: define checker mutation"); const w = beginCandidate(f, "products/lamp.md", "# Lamp\n"); const result = run(f, ["finish", "--preview", "--worktree", w, "--message", "x"]); expect(git(w, "rev-parse", "HEAD")).toBe(git(f.vault, "rev-parse", "HEAD")); expect(readFileSync(join(w, "README.md"), "utf8")).toBe("checker mutation\n"); return result }, cause: "DOMAIN_PATH_SET_MISMATCH", exit: 3, nextAction: PREVIEW },
	...
	{ name: "apply integration unproved after post-merge changes read-back", run: () => { const f = fixture(); const w = beginCandidate(f); const id = previewCandidate(f, w); const candidateCommit = git(w, "rev-parse", "HEAD"); write(f.vault, ".git/hooks/post-merge", "#!/bin/sh\ngit commit --allow-empty -m 'test: post merge'\n"); chmodSync(join(f.vault, ".git/hooks/post-merge"), 0o700); const failed = run(f, ["finish", "--apply", "--preview-id", id, "--worktree", w]); expect(git(f.vault, "rev-parse", "HEAD")).not.toBe(candidateCommit); expect((data(run(f, ["inspect", "--worktree", w])).recovery as { state: string }).state).toBe("recoverable"); expect(run(f, ["recover", "--worktree", w]).envelope?.result.causeCode).toBe("SUCCESS_COMPLETED"); return failed }, cause: "INTERNAL_INTEGRATION_UNPROVED", exit: 1, handoffOwner: "operator", transaction: "unknown" },
	{ name: "apply integration unproved with a failing lock release", run: () => { const f = fixture(); const w = beginCandidate(f); const id = previewCandidate(f, w); git(f.vault, "-c", "core.hooksPath=/nonexistent", "branch", "unrelated", f.initialHead); const lock = join(f.vault, ".git", "vault-note-commits.lock"); write(f.vault, ".git/hooks/post-merge", '#!/bin/sh\ngit symbolic-ref HEAD refs/heads/unrelated\n/bin/chmod 000 "$FAULT_LOCK"\n'); chmodSync(join(f.vault, ".git/hooks/post-merge"), 0o700); try { return run(f, ["finish", "--apply", "--preview-id", id, "--worktree", w], { FAULT_LOCK: lock }) } finally { if (existsSync(lock)) chmodSync(lock, 0o700) } }, cause: "INTERNAL_INTEGRATION_UNPROVED", exit: 1, handoffOwner: "operator", transaction: "unknown" },
