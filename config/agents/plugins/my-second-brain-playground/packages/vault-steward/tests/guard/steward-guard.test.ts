...
test("V2 fixture 2 / B5: a hook denying the completion ref refuses preview before the commit and apply before the lock", () => {
	...
	expect(previewed.stderr).toBe("")
	expect(previewed.envelope?.result).toMatchObject({ causeCode: "DOMAIN_PRECONDITION_UNMET", transactionState: "unchanged", effects: { completed: [], remaining: ["candidate.commit", "preview.record"], uncertain: [], inventoryComplete: true }, nextAction: "vault-steward.inspect", repairAction: "Run 'bun run guard:install' in the vault, then rerun the same command." })
	expect(previewed.envelope?.message).toMatch(/^DOMAIN_GUARD_INCOMPATIBLE: the installed reference-transaction hook denies refs\/vault-note-commits\//)
	...
	expect(applied.stderr).toBe("")
	expect(applied.envelope?.message).toMatch(/^DOMAIN_GUARD_INCOMPATIBLE/)
...
test("human mode prints warnings to stderr and one refusal line; machine mode keeps stderr empty", () => {
	...
	expect(refusal.stderr.startsWith("DOMAIN_PRECONDITION_UNMET: DOMAIN_CANDIDATE_NOT_FOUND")).toBe(true)
