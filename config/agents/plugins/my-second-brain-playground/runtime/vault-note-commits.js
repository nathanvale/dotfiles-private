...
function configuredVault() {
  ...
  if (typeof payload !== "object" || payload === null || Array.isArray(payload) || payload.schemaVersion !== 1 || typeof payload.vault !== "string" || !isAbsolute(payload.vault) || Object.keys(payload).sort().join(",") !== "schemaVersion,vault") {
...
function stateRoot(command) {
  const root = process.env.XDG_STATE_HOME ?? (process.env.HOME ? join(process.env.HOME, ".local", "state") : "");
  if (!root || !isAbsolute(root))
    refuse(command, "STATE_HOME_MISSING", null, "Set an absolute XDG_STATE_HOME or HOME, then retry.");
  return join(root, "my-second-brain", "vault-note-commits");
}
function receiptPath(worktree) {
...
function atomicPrivateJson(path, payload) {
...
function begin(args) {
  ...
  if (vaultValues.length > 1)
    refuse("begin", "INVALID_USAGE", null, "Provide --vault at most once.");
...
function checkWhitespace(manifest, committed) {
...
function complete(manifest, commit) {
  ...
  try {
    ...
    atomicPrivateJson(receipt, { ...manifest, code, ...commit ? { commit } : {} });
  } catch {
    preserve(manifest, "COMPLETION_RECORD_FAILED", commit ? "The commit reached main but its receipt could not be saved. Inspect main and the preserved candidate before retrying." : "No note changed but its receipt could not be saved. Preserve the candidate and inspect local state before retrying.", false, commit);
  }
  const removed = run(["git", "--no-optional-locks", "worktree", "remove", manifest.worktree], manifest.vault);
  return outcome(true, "finish", code, manifest.runId, removed.exitCode === 0 ? commit ? "Run remote sync separately when you want to publish main." : "No candidate changes were authored. This does not verify the freshness of canonical notes." : "Completion is recorded. Inspect the retained candidate before removing it.", {
...
`))
  ...
  if (!isAbsolute(worktree) || resolve(worktree) !== worktree)
    refuse("finish", "INVALID_USAGE", null, "Use the exact absolute worktree path returned by begin.");
