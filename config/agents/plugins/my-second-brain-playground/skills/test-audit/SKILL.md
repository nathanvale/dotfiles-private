---
name: test-audit
description: "Audit tests when writing, changing, or reviewing My Second Brain Playground plugin tests. Gate new tests and find low-value, implementation-coupled, or duplicative coverage and test-only production seams."
---
...
Read repository and scoped `AGENTS.md` files first. For a plugin test change,
follow the repository's `docs/agents/coding-standards.md` and the `test-design`
brief before editing. This skill supplies the audit and authoring value bar;
the owning repository supplies commands and quality gates.
...
divide read-only discovery by owner when useful:

- plugin packages (`packages/`);
- skills, hooks, and runtime scripts;
- CLI and tooling;
...
Never edit source or tests while the test runner is active in the checkout.
Use the owning package's scripts and the repository's required gates.

1. Run the smallest owner and sibling tests through their package scripts.
...
4. Run the owning package's `bun run typecheck`, `bun run lint`, and
   `bun run test`; then run the repository's required changed-code gates.
5. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.
6. Review the final diff against the candidate ledger and retained contracts.

## Landing and continuation

Follow the owning repository's branch, review, and commit rules. Complete one
coherent batch at a time. Refresh the baseline before another audit batch.
