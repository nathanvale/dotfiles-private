...
function isWeekPath(pathname) {
  return pathname === '/' || pathname === '/week';
}

function isNewPath(pathname) {
  return pathname === '/new';
}

function isSavedDraftPath(pathname, state) {
  return state.draft && pathname === `/draft/${state.draft.id}`;
}

function isSavePath(pathname) {
  return pathname === '/save';
}
...
function serveWeek({ response, state }) {
  respondHtml(response, state, overview(state));
}

function serveNewDraft({ response, state }) {
  respondHtml(response, state, newDraft());
}

function serveSavedDraft({ response, state }) {
  respondHtml(response, state, savedDraft(state.draft));
}

async function saveDraft({ request, response, state, statePath }) {
  if (state.draft) {
...
function serveSubmit({ response, state }) {
  respondHtml(response, state, '<p role="alert">Submission is unavailable in this fixture.</p>', 405);
}

function serveNotFound({ response, state }) {
  respondHtml(response, state, '<p>Page not found.</p>', 404);
}

const FIXTURE_ROUTES = [
  { method: 'GET', matches: isWeekPath, handle: serveWeek },
  { method: 'GET', matches: isNewPath, handle: serveNewDraft },
  { method: 'GET', matches: isSavedDraftPath, handle: serveSavedDraft },
  { method: 'POST', matches: isSavePath, handle: saveDraft },
  { matches: isSubmitPath, handle: serveSubmit },
];

function findFixtureRoute(method, pathname, state) {
  for (const route of FIXTURE_ROUTES) {
    if (route.method !== undefined && route.method !== method) continue;
    if (route.matches(pathname, state)) return route;
  }
  return undefined;
}

async function handleFixtureRequest(request, response, statePath) {
  ...
  const route = findFixtureRoute(request.method, url.pathname, state);
  if (route) return route.handle({ request, response, state, statePath });
  serveNotFound({ response, state });
}

function fixtureRequestHandler(statePath, request, response) {
  return handleFixtureRequest(request, response, statePath);
}

function fixtureServer(statePath) {
  return createServer(fixtureRequestHandler.bind(null, statePath));
}
