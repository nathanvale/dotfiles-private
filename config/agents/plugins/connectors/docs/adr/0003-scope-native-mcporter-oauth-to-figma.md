...
- Locally observed on 24 September 2026 through this exact candidate: attended `provider-route figma -- auth --no-browser --json` exited 0 after the browser flow completed, then `provider-route figma -- list --schema --json` exited 0 using the cached grant and exposed only `whoami`. OAuth URLs, callback values, and credentials were kept in private runtime output and are not recorded here.
