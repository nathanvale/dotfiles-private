...
export type DownloadTiming = Readonly<{ headerMs: number; idleMs: number; totalMs: number }>;
export type ReleaseSource = { localReleaseOrigin?: string; timing?: DownloadTiming };

const LIMITS = { package: 48 * 1024 * 1024, archive: 48 * 1024 * 1024, manifest: 128 * 1024, signature: 4096 } as const;
// Connect and headers get a fixed deadline; a body that keeps arriving is only
// aborted after an idle gap, or at a total cap that still admits the 48 MiB
// package at roughly 55 KB/s.
const TIMING: DownloadTiming = { headerMs: 45_000, idleMs: 30_000, totalMs: 15 * 60_000 };
...
async function writeBoundedBody(body: ReadableStream<Uint8Array>, descriptor: number, maximum: number, onProgress: () => void): Promise<{ size: number; digest: string } | null> {
...
async function saveBody(response: Response, file: string, maximum: number, onProgress: () => void, sha256?: string): Promise<DownloadFailure["reason"] | null> {
	...
	try {
		const written = await writeBoundedBody(response.body, descriptor, maximum, onProgress);
...
async function download(url: string, file: string, maximum: number, timing: DownloadTiming, sha256?: string): Promise<DownloadFailure["reason"] | null> {
	const controller = new AbortController();
	const abort = () => controller.abort();
	const total = setTimeout(abort, timing.totalMs);
	let stall = setTimeout(abort, timing.headerMs);
	const progressed = () => {
		clearTimeout(stall);
		stall = setTimeout(abort, timing.idleMs);
	};
	try {
		const response = await fetchApproved(url, controller.signal);
		if (typeof response === "string") return response;
		progressed();
		return await saveBody(response, file, maximum, progressed, sha256);
	} catch {
...
export async function downloadAndInstallOp(source?: ReleaseSource): Promise<OpInstallResult | DownloadFailure> {
	...
	try {
		...
		const failure = await download(url, file, LIMITS.package, source?.timing ?? TIMING, OP_RELEASE.sha256);
...
export async function downloadAndInstallMise(source?: ReleaseSource): Promise<MiseInstallResult | DownloadFailure> {
	...
	try {
		...
		for (const [url, file, maximum, digest] of [[manifestUrl, manifestFile, LIMITS.manifest, undefined], [signatureUrl, signatureFile, LIMITS.signature, undefined], [archiveUrl, archiveFile, LIMITS.archive, MISE_RELEASE.archiveSha256]] as const) {
			const failure = await download(url, file, maximum, source?.timing ?? TIMING, digest);
			if (failure) return { ok: false, reason: failure };
		}
