...
import { installVerifiedMise, MISE_RELEASE } from "../bin/setup/mise.ts";
...
test("a redirected state directory refuses without touching its referent", () => {
	...
	try {
		...
		expect(installVerifiedMise({ ...fixture(root), stateDirectory })).toEqual({ ok: false, reason: "state-invalid" });
		const child = invoke(root, { ...fixture(root), stateDirectory }, xdg);
		expect(child.status).toBe(0);
		expect(child.stderr).toBe("");
