...
export const OPTIONS = [
	...
	{ name: "--no-browser", valueName: null, summary: "Print the authorization URL instead of opening the system browser" },
] as const;
...
export const REPAIR = {
	...
	busy: `Another process holds the refresh lock for this account; retry after ${RETRY_DELAY_MS} ms`,
...
export const STATIONS: readonly Station[] = [
	...
	station("canva-auth.login", "SUCCESS_COMPLETED", "external", "the browser callback returned a code and the token exchange stored a session; with --no-browser the authorization URL was written before the wait (stdout in human mode, one stderr line in machine mode)", next(STATUS_ACTION), null),
	station("canva-auth.login", "USAGE_INVALID_INVOCATION", "external", "--account missing or not a lowercase slug", next(HELP_ACTION), REPAIR.usage),
	...
	station("canva-auth.login", "DOMAIN_PRECONDITION_UNMET", "external", "discovery, client registration, or the callback did not meet the contract", next(STATUS_ACTION), REPAIR.precondition),
	...
	station("canva-auth.login", "DOMAIN_RECOVERY_HANDOFF_REQUIRED", "external", "consent completed but the token exchange failed or the account's session lock was held", handoff("human", "A grant may exist at Canva with no local session."), REPAIR.exchange),
	...
	station("canva-auth.logout", "TRANSIENT_NOT_STARTED", "external", "another process holds the account's refresh lock", next(`retry canva-auth logout after ${RETRY_DELAY_MS} ms`), REPAIR.busy),
