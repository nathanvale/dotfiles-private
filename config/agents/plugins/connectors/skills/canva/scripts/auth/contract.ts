...
export const REPAIR = {
	...
	bindingInvalid: "Restore the original matching oauth.json and private registration.json and retry logout; if that is impossible, revoke this client's access in Canva connected apps, stop Provider processes for this account, then have the operator remove only this account's private session.json and registration.json before a new attended login",
	logoutPrecondition: "Restore the matching registered client secret and configuration, or follow the binding-specific refusal's repair action; the stored session was preserved",
...
export const STATIONS: readonly Station[] = [
	...
	station("canva-auth.logout", "DOMAIN_PRECONDITION_UNMET", "external", "a stored registered client secret is unavailable or the stored session binding is incompatible", next(STATUS_ACTION), REPAIR.logoutPrecondition),
