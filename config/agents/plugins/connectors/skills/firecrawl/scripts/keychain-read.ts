// Firecrawl custody's one Keychain process: the fixed macOS security tool, run
// with the caller's argv and environment. bin/one-password-custody.ts owns
// the mapping. Routine tests replace this leaf only inside a copied plugin
// root, with the test-owned reader fake.
