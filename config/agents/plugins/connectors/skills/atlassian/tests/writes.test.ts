...
describe("effects and read-back", () => {
	...
	test("read-back evidence is found, absent, or indeterminate, and only a monotonic revision proves absence after a send", () => {
		const created = readBackEvidence("issue.create", CREATE, never, { issues: [{ key: "PROJ-8", fields: { summary: "Other" } }, { key: "PROJ-9", fields: { summary: "billing   BROKEN" } }] });
		expect(created).toEqual({ kind: "found", effects: [{ kind: "jira-issue", id: "PROJ-9" }] });
		expect(readBackEvidence("issue.create", CREATE, never, { issues: [{ key: "OTHER-9", fields: { summary: "Billing broken" } }] })).toEqual({ kind: "indeterminate", reason: "a matching issue search result carries no stable issue key" });
