...
describe("translateFailure", () => {
	...
	const TABLE: [string, ObservedFailure, ProviderFailureCause][] = [
		...
		["confluence missing content, despite its permission clause (observed live after a delete)", toolError("Failed to retrieve page by ID '123': Error retrieving page content: There is no content with the given id, or the calling user does not have permission to view the content"), "not-found"],
