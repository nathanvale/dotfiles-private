...
const toolError = (): ObservedFailure => ({ kind: "tool-error", contentObserved: true });
const malformed = (message: string): ObservedFailure => ({ kind: "malformed", message, contentObserved: false });

describe("translateFailure", () => {
	// Test-owned table: observed process or tool failures and their closed causes.
	const TABLE: [string, ObservedFailure, ProviderFailureCause][] = [
		["tool error without a structured cause", toolError(), "failed-unknown"],
		...
		["permission wording", process_("You do not have permission to view this issue"), "refused-auth"],
		["not found", process_("HTTP 404 Not Found"), "not-found"],
		["unknown tool", process_("unknown tool getJiraIssue"), "capability-unavailable"],
		["tool not exposed", process_("tool searchConfluence is not exposed on this server"), "capability-unavailable"],
		...
		["auth beats not-found", process_("401 Unauthorized: issue not found"), "refused-auth"],
		["not-found beats capability", process_("unknown tool not found"), "not-found"],
	];
	...
	test("every non-precondition cause carries a null hint", () => {
		for (const observed of [toolError(), process_("HTTP 404 Not Found"), process_("unknown tool"), process_("connect ECONNRESET"), malformed("x"), process_("boom")]) {
	...
	test("content observed is carried through unchanged", () => {
		...
		expect(translateFailure(toolError()).contentObserved).toBe(true);
	});
	...
	test("hostile provider text never survives translation on any path", () => {
		...
		for (const observed of [process_(leak), process_("", leak), toolError(), malformed(leak), process_(`atlassian-provider:error:credential-invalid:${leak}`, "", 4), process_(`${leak} HTTP 401`), process_(`${leak} 404 not found`)]) {
