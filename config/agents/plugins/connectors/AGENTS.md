...
- Canva's OAuth grant lives in MCPorter's native vault under one private root per Canva Account. Its current owner is the custody interface [`skills/canva/scripts/custody/index.ts`](skills/canva/scripts/custody/index.ts), composed by the packaged adapter [`skills/canva/adapter.ts`](skills/canva/adapter.ts) behind the front door's `bin/connectors auth`, `run`, and `schema`. Read proposed [`docs/adr/0004-move-canva-custody-into-mcporter-native-vault.md`](docs/adr/0004-move-canva-custody-into-mcporter-native-vault.md) before changing it. ADR 0004 stays proposed, and ADR 0002 unchanged, until Nathan accepts it.
