...
- Credential custody stays below MCPorter for static-credential Providers (Atlassian, Mermaid, and configured Context7 and Firecrawl account keys).
- Provider and custody child processes start only as internal roles of the compiled `bin/connectors`, registered by a packaged adapter; their skill scripts are imported modules.
...
- Canva's OAuth grant lives in MCPorter's native vault under one private root per Canva Account. Its current owner is the custody interface [`skills/canva/scripts/custody/index.ts`](skills/canva/scripts/custody/index.ts), composed by the packaged adapter [`skills/canva/adapter.ts`](skills/canva/adapter.ts) behind the front door's `bin/connectors auth`, `run`, and `schema`. Read accepted [`docs/adr/0004-move-canva-custody-into-mcporter-native-vault.md`](docs/adr/0004-move-canva-custody-into-mcporter-native-vault.md) before changing it.
...
- Shared Provider plumbing lives in `bin/`: `provider-process.ts` (refusal prefix, PATH lookup, scrub, exec), `private-state.ts` (owned 0700 directories, exact-0600 files), `one-password-custody.ts` (scoped item reads), and `account-key/` (optional account registration and Provider relay). A skill's custody module is imported only through its `index.ts`.
...
1. Choose the shape. A keyless hosted MCP endpoint with an optional account key (Context7, Firecrawl) declares keyless and `<id>-account` Provider entries with identical allow-lists, plus an adapter built from `bin/account-key/`; test both modes through `tests/account-key.test.ts`. Figma's hosted native OAuth shape adds an attended `auth` route; read ADR 0003 before copying it. A credentialed service (Atlassian) adds one Provider module below MCPorter, run as its packaged adapter's internal Provider role, and a semantic dispatcher that owns Route Selection, the trusted-origin binding, and any write policy. A per-user OAuth service (Canva) declares a dispatcher-owned MCPorter OAuth route, a client-mode switch, and a packaged adapter whose custody interface places each account's MCPorter vault under a private root; read ADR 0004 before changing it.
2. Copy the closest existing skill as the template: `skills/context7/` for the keyless or optional-key shape, `skills/atlassian/` for the credentialed shape, `skills/canva/` for the OAuth shape. Rename, then remove everything the new service does not need.
