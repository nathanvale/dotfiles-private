...
- Credential custody stays below MCPorter for static-credential Providers (Atlassian and Mermaid).
...
- Shared Provider plumbing lives in `bin/`: `provider-process.ts` (refusal prefix, PATH lookup, scrub, exec), `private-state.ts` (owned 0700 directories, exact-0600 files), and `one-password-custody.ts` (scoped item reads). A skill's custody module is imported only through its `index.ts`.
