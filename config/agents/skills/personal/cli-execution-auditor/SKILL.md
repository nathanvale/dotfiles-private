...
- Findings ledger owner: `src/ledger/` owns the complete auditor-local findings lifecycle and Markdown format.

## Ledger

Auditor-local findings ledger at `src/ledger/`. It owns the complete
auditor-local findings lifecycle and Markdown format, including Open Findings /
Finding History, states, signature dedupe, and never-delete history.
...
- Run `./config/agents/skills/personal/test-runner/src/test-runner.sh run --cwd config/agents/skills/personal/cli-execution-auditor -- src/**/*.test.ts` after engine, clause, or ledger changes.
