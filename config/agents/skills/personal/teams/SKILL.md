...
| Argument | Meaning |
|---|---|
| `<conversation>` | Channel or person name as it appears in Teams (`"Project Devs"`, `"Alex Example"`). Resolved against the local cache; ambiguous or unknown names abort and print the candidates. |
...
```bash
teams-send "Project Devs" ./message.txt        # stage only — you press Enter
teams-send "Alex Example" ./msg.txt --send       # stage and send
echo "quick note" | teams-send "Jordan Example" -        # from stdin
```
