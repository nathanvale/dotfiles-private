...
1. No command, dashboard/front-door requested, or inbox state unclear -> **dashboard** - run `bun run config/agents/skills/personal/skill-feedback/src/skill-feedback-runner.ts`; treat output as a launcher; use `health` for JSON.
...
7. Evidence review requested -> review - run `bun run config/agents/skills/personal/skill-feedback/src/skill-feedback-runner.ts review`; add `--plain` for human reading.
8. Blocked correlation witness diagnostics -> correlate preview - run `bun run config/agents/skills/personal/skill-feedback/src/skill-feedback-runner.ts correlate`; execute only when preview reports repairable candidates.
9. Retention cleanup requested -> purge preview - inspect help first; execute only after preview plus explicit delete authority.
10. Dashboard, reports, report, usage, or queue source change requested -> source-change - read `config/agents/skills/personal/skill-feedback/docs/plans/2026-07-02-001-feat-skill-feedback-human-observability-mvp-plan.md`, then `AGENTS.md` and `ARCHITECTURE.md`.
...
- Use help when command flags or execution detail are needed: `bun run config/agents/skills/personal/skill-feedback/src/skill-feedback-runner.ts --help`.
...
- Run package tests: `config/agents/skills/personal/test-runner/src/test-runner.sh run --cwd config/agents/skills/personal/skill-feedback -- src`.
...
- No-arg front-door spec: `config/agents/skills/personal/skill-feedback/docs/plans/2026-07-02-002-spec-skill-feedback-no-arg-front-door.md`.
- Human observability MVP plan: `config/agents/skills/personal/skill-feedback/docs/plans/2026-07-02-001-feat-skill-feedback-human-observability-mvp-plan.md`.
