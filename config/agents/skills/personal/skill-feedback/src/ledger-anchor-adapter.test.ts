...
describe("skill-feedback U3 ledger anchor adapter", () => {
	test("canonicalizes, sorts, and de-duplicates repo path anchors", () => {
		const facts = deriveLedgerAnchorFacts({
			...
			touched_surfaces: [
				{
					type: "path",
					value: "config/agents/skills/personal/skill-feedback/src/../src/skill-feedback-runner.ts",
				},
				{
					type: "path",
					value: "config/agents/skills/personal/skill-feedback/CONTEXT.md",
				},
				{
					type: "path",
					value: "config/agents/skills/personal/skill-feedback/src/./skill-feedback-runner.ts",
				},
			],
			observations: [],
		});
		...
		expect(facts).toMatchObject({
			...
			ledger_anchor_key:
					"path:config/agents/skills/personal/skill-feedback/CONTEXT.md|config/agents/skills/personal/skill-feedback/src/skill-feedback-runner.ts",
		});
	...
	test("uses touched surface paths before observation target paths", () => {
		const facts = deriveLedgerAnchorFacts({
			...
			touched_surfaces: [
				{ type: "path", value: "config/agents/skills/personal/skill-feedback/CONTEXT.md" },
			],
			observations: [
				{
					kind: "tool_failure",
		...
		expect(facts.owner_paths).toEqual(["config/agents/skills/personal/skill-feedback/CONTEXT.md"]);
		expect(facts.attempted_targets).toEqual([
				{ type: "path", value: "config/agents/skills/personal/skill-feedback/CONTEXT.md" },
		]);
		expect(facts.ledger_anchor_key).toBe(
			"path:config/agents/skills/personal/skill-feedback/CONTEXT.md",
		);
	});
	...
	test("falls back to observation target paths when touched paths are absent", () => {
			...
			observations: [
				{
					...
					target: {
						type: "path",
						value: "config/agents/skills/personal/skill-feedback/references/report-shape.md",
					},
		...
		expect(facts).toMatchObject({
			anchor_strength: "strong_path",
			owner_paths: ["config/agents/skills/personal/skill-feedback/references/report-shape.md"],
			ledger_anchor_key:
				"path:config/agents/skills/personal/skill-feedback/references/report-shape.md",
		});
	});
	...
	test("keeps non-anchor report fields out of the ledger anchor key", () => {
		const base = {
			...
			touched_surfaces: [
				{ type: "path" as const, value: "config/agents/skills/personal/skill-feedback/CONTEXT.md" },
			],
			observations: [
				{
