...
async function runQueueOwnerPathStrong(
	...
	return coverIgnoredGitStation(station, {
		prepare: async (root) => {
			await writeInboxReport(
				...
				v1HumanReport({
					...
					touchedSurfaces: [{ type: "path", value: "config/agents/skills/personal/skill-feedback/SKILL.md" }],
				}),
			);
		},
		args: ["queue", "--json"],
		assert: ({ envelope }) => {
			...
			expect(rows[0]).toMatchObject({
				target: "config/agents/skills/personal/skill-feedback/SKILL.md",
				evidence_strength: "strong",
			});
		},
	});
...
async function writeWeakQueueFixture(root: string): Promise<void> {
	await writeInboxReport(
		...
		v1HumanReport({
			...
			touchedSurfaces: [
				{ type: "path", value: "config/agents/skills/personal/skill-feedback/SKILL.md" },
			],
		}),
	);
}
