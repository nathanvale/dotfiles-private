...
const contactMap = new Map<string, string>([["+61400000000", "Alex"]]);
...
describe("parseRow", () => {
	test("splits text-only rows into a single text part", () => {
		...
		expect(parts[0]?.contact_name).toBe("Alex");
	});
...
describe("utility helpers", () => {
	...
	test("normalizes Australian phone numbers and contact lookup", () => {
		expect(normalizePhone("0400 000 000")).toBe("+61400000000");
		expect(formatContactName("Alex", "Vale", null)).toBe("Alex Vale");
		expect(
			resolveContact("0400 000 000", new Map([["+61400000000", "Alex"]])),
		).toBe("Alex");
	});
...
function makeV2Input(overrides: Partial<V2NoteInput> = {}): V2NoteInput {
	return {
		...
		thread: "Alex",
		is_group: false,
		conversation_with: "Alex",
		conversation_type: "direct",
		text: "Hey, are you picking Casey up from school today?",
		...overrides,
	};
}
...
describe("saveMessageAsMarkdownV2", () => {
	test("writes v2 note with correct frontmatter (fx-01)", () => {
		...
		expect(content).toContain('conversation_with: "Alex"');
		...
		expect(content).toContain(
			"Hey, are you picking Casey up from school today?",
		);
	});
	...
	test("sorts comma-separated unnamed group chat names deterministically (fx-09)", () => {
		...
		const filePath = saveMessageAsMarkdownV2(
			makeV2Input({
				...
				from: "Dave",
				handle: "+61498765432",
				thread: "Dave, Alex, Sarah",
				is_group: true,
				conversation_with: "Dave, Alex, Sarah",
		...
		const content = readFileSync(filePath as string, "utf-8");
		expect(content).toContain('conversation_with: "Dave, Alex, Sarah"');
	...
	test("idempotent re-sync overwrites same path (fx-12)", () => {
		...
		const path2 = saveMessageAsMarkdownV2(
			makeV2Input({
				text: "Hey, are you picking Casey up from school today? (edited: or should I?)",
				edited: true,
	...
	test("writes attachment section but no Attachment Text section", () => {
		...
		const filePath = saveMessageAsMarkdownV2(
			makeV2Input({
				...
				text: "Look what Casey drew at school today!",
				attachments: [att],
			}),
			dir,
		);
...
describe("extractCommitmentCandidates", () => {
	test("detects outbound promise from fx-10 fixture", () => {
		...
		expect(candidates[0]?.conversation_with).toBe("Alex");
		expect(candidates[0]?.quote).toBe("I'll ask Morgan now");
	...
	test("detects outbound offer", () => {
		const msg = makeParsedMessage({
			text: "I can pick up Casey from school",
			is_from_me: true,
			contact_name: "Alex",
		});
		const candidates = extractCommitmentCandidates([msg]);
