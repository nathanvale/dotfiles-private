...
const DOTFILES_ROOT = join(SKILL_ROOT, "..", "..", "..", "..", "..");
...
async function checkTests(): Promise<Finding> {
	...
	return code === 0
		...
		: {
				...
				autoRepairable: false,
					nextAction:
					"human: run config/agents/skills/personal/test-runner/src/test-runner.sh run --cwd config/agents/skills/personal/classic-cinema -- src/*.test.ts and fix",
			};
}
...
async function checkProductivityYml(): Promise<Finding> {
	const candidates = [
		...
		join(DOTFILES_ROOT, "..", "my-second-brain", ".productivity.yml"),
...
async function checkOwnerPaths(): Promise<Finding> {
	...
	// This skill's own owner files appear as config/agents/skills/personal/classic-cinema/<references|src>/<file>
	...
	// dotfiles root (five levels above SKILL_ROOT, i.e. config/agents/skills/personal/classic-cinema). Skip
	// glob/placeholder forms (src/*.ts, src/<command>.ts) — they are illustrative.
	const repoRoot = DOTFILES_ROOT;
	const referenced = new Set(
		[
			...md.matchAll(
				/\bconfig\/agents\/skills\/personal\/classic-cinema\/(?:references|src)\/[A-Za-z0-9._/-]+\.[A-Za-z0-9]+/g,
			),
		]
			.map((m) => m[0])
			.filter((rel) => !/[*<>]/.test(rel)),
