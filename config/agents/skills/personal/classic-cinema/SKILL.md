...
4. Health check — `bun run config/agents/skills/personal/classic-cinema/src/heal-skill.ts check`.

## Owner

- Runtime: **Bun**. Commands live at `config/agents/skills/personal/classic-cinema/src/*.ts`; run with `bun run config/agents/skills/personal/classic-cinema/src/<command>.ts`. Each command's `--help` is the source of truth for flags, stdout/stderr, temp files, and exit codes — do not copy them here.
- Shared API client + types (base URL, fetch+cache, AEST time, seatmap shapes): `config/agents/skills/personal/classic-cinema/src/cinema-api.ts`.
- Booking-log model + validation: `config/agents/skills/personal/classic-cinema/src/booking-log.ts`.
- Booking choreography and API details: `config/agents/skills/personal/classic-cinema/references/booking-flow.md`.
- Argument parsing: `config/agents/skills/personal/classic-cinema/references/arg-parsing.md`.
- Email template fill: `config/agents/skills/personal/classic-cinema/references/template-fill.md`.
- Email sending: `config/agents/skills/personal/classic-cinema/references/email-send.md`.
- Booking log shape: `config/agents/skills/personal/classic-cinema/references/booking-log.md`.
- Skill health doctor: `config/agents/skills/personal/classic-cinema/src/heal-skill.ts` (run `heal-skill check` when a booking fails or output looks wrong).
- Legacy Python scripts under `scripts/*.py` are superseded by `src/*.ts`; retirement criteria: `config/agents/skills/personal/classic-cinema/references/retirement-criteria.md`.
...
| Step | Command |
|------|---------|
| Listing / details | `bun run config/agents/skills/personal/classic-cinema/src/list-movies.ts [--movie QUERY]` |
| Availability | `bun run config/agents/skills/personal/classic-cinema/src/check-availability.ts --session-ids ID[,ID]` |
| Tickets + pricing | `bun run config/agents/skills/personal/classic-cinema/src/parse-tickets.ts --session-id ID --spec "1+1"` |
| Seat pick | `bun run config/agents/skills/personal/classic-cinema/src/pick-seats.ts --seatmap-file PATH --zone ZONE --count N` |
| Fill email | `bun run config/agents/skills/personal/classic-cinema/src/fill-ticket.ts …` (then send via `gog`, see [email-send.md](references/email-send.md)) |
| Health doctor | `bun run config/agents/skills/personal/classic-cinema/src/heal-skill.ts check` |
...
- After any `src/` change: `config/agents/skills/personal/test-runner/src/test-runner.sh run --cwd config/agents/skills/personal/classic-cinema -- src/cinema-api.test.ts src/pick-seats.test.ts src/fill-ticket.test.ts src/booking-log.test.ts` and `cd config/agents/skills/personal/classic-cinema && bunx tsc --noEmit -p tsconfig.json`.
- After any change: `bunx biome check --diagnostic-level=error config/agents/skills/personal/classic-cinema/src/`.
- Whole-skill health (scripts, frozen template, booking log, owner paths): `bun run config/agents/skills/personal/classic-cinema/src/heal-skill.ts check`.
