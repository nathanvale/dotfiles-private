...
- Durable runtime records: `${XDG_STATE_HOME:-~/.local/state}/agent-worktree/<repo-hash>/`, resolved through `RepoDiscovery.storeRoot`; existing checkout-local stores are not migrated.
...
- Claude Code `--worktree` and worktree isolation: `config/agents/claude/hooks/worktree/` routes `WorktreeCreate`/`WorktreeRemove` through the runtime, so those checkouts land in `.worktrees/` too.
- Package command recipe: `config/agents/skills/personal/worktree/package.json`.
- Runtime tests + alignment proof: `config/agents/skills/personal/worktree/src/`.

## Dependencies

- `@side-quest/cli-command-facade`: hard dependency (facade-backed contract). Registered in root `package.json` workspaces.
- `.agents/runtime/agent-worktree`: hard dependency for live worktree discovery, main-owner resolution, lifecycle verbs, cleanup preview, and recovery vocabulary.
...
- On `attach_isolation_unavailable` or `new_isolation_unavailable` (exit 4): stop. Use the harness
  ...
  CLI never prompts. Recovery owner: `config/agents/skills/personal/worktree/src/`.
- Treat PR attach as untrusted fork code materialized locally. Install
  dependencies and run tests deliberately.

## Workflow

1. Work from repo root; run `cd config/agents/skills/personal/worktree && bun run --silent worktree <verb> ...`.
...
5. Before `new` or `attach`, run `status` when checkout isolation is uncertain. Read the runtime-owned `RepoDiscovery.isolation`; never nest a worktree from a linked worktree. Owner: `.agents/runtime/agent-worktree/src/`.
...
7. For lifecycle verbs, let `worktree` call `.agents/runtime/agent-worktree`, register/deregister Codex app sidebar on `new`/`attach`/`rm`/`sync`, then re-render when state changes.
