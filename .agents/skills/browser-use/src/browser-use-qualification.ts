...
const QUALIFICATION_FRONT_DOOR =
	".agents/skills/browser-use/src/browser-use-bun-preflight.ts" as const;
...
const BROWSER_USE_QUALIFICATION_SUPPLEMENTAL_INVENTORY = [
	".agents/skills/browser-use/package.json",
	".agents/skills/browser-use/src/browser-use-bun-preflight.ts",
...
export type BrowserUseQualificationManifest = {
	...
	front_door: {
		path: ".agents/skills/browser-use/src/browser-use-bun-preflight.ts";
...
async function buildCanonicalExecutionClosure(): Promise<ExecutionClosureOwner[]> {
	...
	const owners = [
		{
			ownerId: "browser-use",
			entrypoint: ".agents/skills/browser-use/src/browser-use.ts",
