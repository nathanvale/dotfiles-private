...
import { compileSpecificationCandidate, GENERATOR_CONTRACT_VERSION, INPUT_SCHEMA_VERSION } from '../src/index.ts'
...
describe('specification digest', () => {
	test('is identical across repeated runs of the same candidate', async () => {
		...
		expect(second.digest.specificationDigest).toBe(first.digest.specificationDigest)
	...
	test('is identical for a cosmetically permuted copy', async () => {
		const original = compileSpecificationCandidate(await readCandidate('vault-git'))
		const permuted = compileSpecificationCandidate(await readPermutedCandidate('vault-git'))
		...
		expect(permuted.digest.specificationDigest).toBe(original.digest.specificationDigest)
	})
	...
	test('differs when a meaning actually changes', async () => {
		...
		const changed = compileSpecificationCandidate(source.replace('"offline_mode"', '"offline_mode_2"'))

		expect(original.ok && changed.ok).toBe(true)
		if (!original.ok || !changed.ok) return
		expect(changed.digest.specificationDigest).not.toBe(original.digest.specificationDigest)
	})
	...
	test('envelope distinguishes input schema version from generator contract version', async () => {
		...
		expect(result.digest.generatorContractVersion).toBe(GENERATOR_CONTRACT_VERSION)
	})
})
