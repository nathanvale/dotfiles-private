...
export async function readPermutedCandidate(product: 'vault-git'): Promise<string> {
	return await Bun.file(new URL(`${product}.permuted.state-machine.jsonc`, PERMUTED_DIR)).text()
}
